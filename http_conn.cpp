...
// 网站的根目录
const char* doc_root = "/home/nowcoder/tinywebserver/resources";
// 定义HTTP响应的一些状态信息
...
void addfd(int epoll_fd, int fd, bool one_shot, bool et){
    ...
    epoll_ctl(epoll_fd, EPOLL_CTL_ADD, fd, &event);//加入红黑树中
...
void modfd(int epoll_fd, int fd, int ev){
    epoll_event event;
    //重新创建一个epoll_event类型的变量用来绑定fd并设置它的event
    event.data.fd = fd;
...
void http_conn::init(int sock_fd, const sockaddr_in& addr){ 
    ...
    // 添加sock_fd到epoll对象中，设置oneshot和ET边沿触发
    ...
    // time函数接收一个指向time_t 类型的指针作为参数timer，用于接收当前时间的时间戳。传递NULL，则time函数会将当前时间的时间戳返回
...
bool http_conn::read(){
    ...
    int bytes_rd = 0;
    while(true){    // m_sock_fd已设置非阻塞，因此recv函数的第四个参数flags不用再设置为MSG_DONTWAIT表示非阻塞，设置为0即可
        bytes_rd = recv(m_sock_fd, m_rd_buf + m_rd_idx, RD_BUF_SIZE - m_rd_idx, 0);   // 第二个参数传递的是缓冲区中开始读入的地址偏移
        if(bytes_rd == -1){
            if(errno == EAGAIN || errno == EWOULDBLOCK){
                break;      // 非阻塞读取，没有数据了，recv函数非阻塞返回-1时，并且错误码是 EAGAIN 或 EWOULDBLOCK，表示缓冲区中没有数据可读，可以继续尝试或者退出
            }
            return false;   // 读取错误，main中调用conn_close()
        }else if(bytes_rd == 0){    
...
http_conn::HTTP_CODE http_conn::process_read(){
    ...
    char* text = 0;
    while((m_check_stat == CHECK_STATE_CONTENT && line_stat == LINE_OK) // 解析到了头部和请求行一行完整的数据，或者解析到了请求体一行完整的数据
        || (line_stat = parse_one_line()) == LINE_OK){                  

        // 获取一行数据
        text = get_line();//char* get_line(){return m_rd_buf + m_line_start;}前面的parse_one_line给我们设置了'\0'，或者头部和请求行给我们设置好了换行
...
http_conn::HTTP_CODE http_conn::parse_request_line(char* text){
    ...
    // 在str1中查找第一个出现在str2中的字符，返回该字符在str1的位置指针
    if(!m_url) return BAD_REQUEST;
    *m_url = '\0';      // GET\0/index.html HTTP/1.1，此时text到\0结束，表示 GET\0
    m_url++;            // 现在m_url指向index前面的单斜线   /index.html HTTP/1.1

    char* method = text;    // GET\0,因此method就是"GET"
    ...
    *m_version = '\0';  // /index.html\0HTTP/1.1，此时m_url到\0结束，表示 /index.html\0
    ...
    if(strncasecmp(m_url, "http://", 7) == 0){
        ...
        // 在str中查找字符c第一次出现的位置，并返回该位置的指针，未出现返回NULL
    }
    // int strncasecmp(const char* str1,const char* str2,size_t n)
    // 比较str1和str2的前n个字符是否相等，忽略大小写差异，相等返回0
    if(!m_url || m_url[0] != '/'){
        return BAD_REQUEST;
    }
...
http_conn::HTTP_CODE http_conn::do_request(){
    ...
    // 判断访问权限
    if ( ! ( m_file_stat.st_mode & S_IROTH ) ) {// S_IROTH 是一个用于表示其他用户（除了文件所有者和文件所属组）对文件的读权限的宏定义
        return FORBIDDEN_REQUEST;
    }

    // 判断是否是目录
    if ( S_ISDIR( m_file_stat.st_mode ) ) { // S_ISDIR 宏定义的值是一个函数，用于检查指定的文件模式是否表示一个目录。
    ...
    // 创建内存映射,m_file_address是客户请求的目标文件被mmap到内存中的起始位置
    m_file_address = ( char* )mmap( 0, m_file_stat.st_size, PROT_READ, MAP_PRIVATE, fd, 0 );
    // addr：指定映射的起始地址，通常设置为 NULL，表示由系统自动选择合适的地址，0和null等效
...
void http_conn::unmap(){
    if(m_file_address){
        ...
        // int munmap(void* addr, size_t length);第一个参数是映射区域的起始地址
...
bool http_conn::write(){
    ...
    while(1) {
        ...
        */
        ...
        }else{                                      // 还没发完头部，更新头部m_iv[0]中下一轮发送的起始下标和长度
        ...
        if (bytes_to_send <= 0){
            // 没有数据要发送了
            unmap();
            modfd(m_epoll_fd, m_sock_fd, EPOLLIN);// 重新加入epoll队列，设置读事件监听

            if (m_linger){// 如果m_linger为true，保持连接，我们重新初始化
                init();
                return true;
            }else{
...
bool http_conn::add_content_type() {    // 响应体类型，当前文本形式，这里我们写死了类型是text/html
...
void http_conn::process(){      // 线程池中线程的业务处理
    ...
    if(!write_ret){
        conn_close();
        if(timer) m_timer_lst.del_timer(timer);  // 失败，移除其对应的定时器
    }
